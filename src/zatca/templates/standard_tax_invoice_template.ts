...
<Invoice xmlns="urn:oasis:names:specification:ubl:schema:xsd:Invoice-2" xmlns:cac="urn:oasis:names:specification:ubl:schema:xsd:CommonAggregateComponents-2" xmlns:cbc="urn:oasis:names:specification:ubl:schema:xsd:CommonBasicComponents-2" xmlns:ext="urn:oasis:names:specification:ubl:schema:xsd:CommonExtensionComponents-2"><ext:UBLExtensions>SET_UBL_EXTENSIONS_STRING</ext:UBLExtensions>
  ...
  <cac:AccountingCustomerParty>
      ...
      <cac:PostalAddress>
        ...
        <cbc:PlotIdentification>SET_BUYER_PLOT_IDENTIFICATION</cbc:PlotIdentification>
...
export enum ZATCAStandardPaymentMethods {
    CASH="10",
    CREDIT="30",
    BANK_ACCOUNT="42",
    BANK_CARD="48"
...
export enum ZATCAStandardInvoiceTypes{
    INVOICE="388",
    DEBIT_NOTE="383",
    CREDIT_NOTE="381"
}
...
export interface ZATCASimplifiedInvoicCancelation{
    ...
    payment_method: ZATCAStandardPaymentMethods,
    cancelation_type: ZATCAStandardInvoiceTypes,
    reason: string
}
...
export default function populate(props: ZATCAStandardInvoiceProps): string {
    ...
    populated_template = populated_template.replace("SET_INVOICE_TYPE", props.cancelation ? props.cancelation.cancelation_type : ZATCAStandardInvoiceTypes.INVOICE);
    ...
    if(props.egs_info.isStandardInvoice){
        populated_template = populated_template.replace("SET_STANDARDINV", "YES");
    }else{
        populated_template = populated_template.replace("SET_STANDARDINV", "NO");
    }
    ...
    populated_template = populated_template.replace("SET_BUYER_NAME", props.egs_info.buyerName);

    populated_template = populated_template.replace("SET_BUYER_STREET_NAME", props.egs_info.buyerStreet);
    populated_template = populated_template.replace("SET_BUYER_BUILDING_NUMBER", props.egs_info.buyerBuilding);
    populated_template = populated_template.replace("SET_BUYER_PLOT_IDENTIFICATION", props.egs_info.buyerPlot_identification);
    populated_template = populated_template.replace("SET_BUYER_CITY_SUBDIVISION", props.egs_info.buyerCity_subdivision);
    populated_template = populated_template.replace("SET_BUYER_CITY", props.egs_info.buyerCity);
    populated_template = populated_template.replace("SET_BUYER_POSTAL_NUMBER", props.egs_info.buyerPostal_zone);
    populated_template = populated_template.replace("SET_BUYER_VAT_NUMBER", props.egs_info.buyerVatNumber);
    populated_template = populated_template.replace("SET_DELIVERY_DATE", props.egs_info.deliveryDate);
    populated_template = populated_template.replace("SET_PAYMENT_MODE", props.egs_info.paymentMode);
    populated_template = populated_template.replace("SET_BUYER_CRN", props.egs_info.buyerCRN);
